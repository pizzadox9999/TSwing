...
public class TAffineTransform implements Cloneable, Serializable {
    ...
    public TAffineTransform(TAffineTransform t) {
        this.type = t.type;
        this.m00 = t.m00;
        this.m10 = t.m10;
        this.m01 = t.m01;
    ...
    public TAffineTransform(double m00, double m10, double m01, double m11, double m02, double m12) {
        this.type = TYPE_UNKNOWN;
        this.m00 = m00;
        this.m10 = m10;
        this.m01 = m01;
    ...
    public void setTransform(TAffineTransform t) {
        type = t.type;
    ...
    public static TAffineTransform getTranslateInstance(double mx, double my) {
        TAffineTransform t = new TAffineTransform();
        t.setToTranslation(mx, my);
        return t;
    }

    public static TAffineTransform getScaleInstance(double scx, double scY) {
        TAffineTransform t = new TAffineTransform();
    ...
    public static TAffineTransform getShearInstance(double shx, double shy) {
        TAffineTransform m = new TAffineTransform();
        m.setToShear(shx, shy);
        return m;
    }

    public static TAffineTransform getRotateInstance(double angle) {
        TAffineTransform t = new TAffineTransform();
        t.setToRotation(angle);
        return t;
    }

    public static TAffineTransform getRotateInstance(double angle, double x, double y) {
        TAffineTransform t = new TAffineTransform();
    ...
    public void translate(double mx, double my) {
        concatenate(TAffineTransform.getTranslateInstance(mx, my));
    }

    public void scale(double scx, double scy) {
        concatenate(TAffineTransform.getScaleInstance(scx, scy));
    }

    public void shear(double shx, double shy) {
        concatenate(TAffineTransform.getShearInstance(shx, shy));
    }

    public void rotate(double angle) {
        concatenate(TAffineTransform.getRotateInstance(angle));
    }

    public void rotate(double angle, double px, double py) {
        concatenate(TAffineTransform.getRotateInstance(angle, px, py));
    }
     ...
     */
    TAffineTransform multiply(TAffineTransform t1, TAffineTransform t2) {
    ...
    public void concatenate(TAffineTransform t) {
        setTransform(multiply(t, this));
    }

    public void preConcatenate(TAffineTransform t) {
    ...
    public TAffineTransform createInverse() throws TNoninvertibleTransformException {
        ...
        if (Math.abs(det) < ZERO) {
            ...
            throw new TNoninvertibleTransformException(Messages.getString("awt.204")); //$NON-NLS-1$
        }
        return new TAffineTransform(
                 m11 / det, // m00
                -m10 / det, // m10
    ...
    public TPoint2D transform(TPoint2D src, TPoint2D dst) {
        if (dst == null) {
            if (src instanceof TPoint2D.TDouble) {
                dst = new TPoint2D.TDouble();
            } else {
                dst = new TPoint2D.TFloat();
            }
        }
    ...
    public void transform(TPoint2D[] src, int srcOff, TPoint2D[] dst, int dstOff, int length) {
        while (--length >= 0) {
            TPoint2D srcPoint = src[srcOff++]; 
            double x = srcPoint.getX();
            double y = srcPoint.getY();
            TPoint2D dstPoint = dst[dstOff]; 
            if (dstPoint == null) {
                if (srcPoint instanceof TPoint2D.TDouble) {
                    dstPoint = new TPoint2D.TDouble();
                } else {
                    dstPoint = new TPoint2D.TFloat();
                }
            }
    ...
    public TPoint2D inverseTransform(TPoint2D src, TPoint2D dst) throws TNoninvertibleTransformException {
        double det = getDeterminant();
        if (Math.abs(det) < ZERO) {
            // awt.204=Determinant is zero
            throw new TNoninvertibleTransformException(Messages.getString("awt.204")); //$NON-NLS-1$
    ...
    public void inverseTransform(double[] src, int srcOff, double[] dst, int dstOff, int length)
        throws TNoninvertibleTransformException
    {
        double det = getDeterminant();
        if (Math.abs(det) < ZERO) {
            // awt.204=Determinant is zero
    ...
    public TShape createTransformedShape(TShape src) {
        if (src == null) {
            return null;
        }
        if (src instanceof TGeneralPath) {
            return ((TGeneralPath)src).createTransformedShape(this);
        }
        TPathIterator path = src.getPathIterator(this);
        TGeneralPath dst = new TGeneralPath(path.getWindingRule());
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof TAffineTransform) {
            TAffineTransform t = (TAffineTransform)obj;
            return
                m00 == t.m00 && m01 == t.m01 &&
                m02 == t.m02 && m10 == t.m10 &&
                m11 == t.m11 && m12 == t.m12;
