...
public class TIndexColorModel extends TColorModel {
    ...
    public TIndexColorModel(int bits, int size, int cmap[], int start,
            int transferType, BigInteger validBits) {

        super(bits, TIndexColorModel.createBits(true),
    ...
    public TIndexColorModel(int bits, int size, int cmap[], int start,
        ...
        super(bits, TIndexColorModel.createBits(hasalpha || (trans >= 0)),
    ...
    public TIndexColorModel(int bits, int size, byte r[], byte g[], byte b[],
            byte a[]) {

        super(bits, TIndexColorModel.createBits(true),
                ColorSpace.getInstance(ColorSpace.CS_sRGB), true, false,
                Transparency.OPAQUE,
                validateTransferType(TColorModel.getTransferType(bits)));
    ...
    public TIndexColorModel(int bits, int size, byte r[], byte g[], byte b[],
        ...
        super(bits, TIndexColorModel.createBits((trans >= 0)),
    ...
    public TIndexColorModel(int bits, int size, byte r[], byte g[], byte b[]) {
        super(bits, TIndexColorModel.createBits(false),
    ...
    public TIndexColorModel(int bits, int size, byte cmap[], int start,
    ...
    public TBufferedImage convertToIntDiscrete(TRaster raster,
            boolean forceARGB) {
        ...
        if (forceARGB || transparency == Transparency.TRANSLUCENT) {
            model = TColorModel.getRGBdefault();
        } else if (transparency == Transparency.BITMASK) {
            model = new TDirectColorModel(25, 0x00ff0000, 0x0000ff00,
                    0x000000ff, 0x01000000);
        } else {
            model = new TDirectColorModel(24, 0x00ff0000, 0x0000ff00, 0x000000ff);
        }

        int w = raster.getWidth();
        int h = raster.getHeight();

        TWritableRaster distRaster = model.createCompatibleWritableRaster(w, h);
        ...
        return new TBufferedImage(model, distRaster, false, null);
    }
    ...
    public TWritableRaster createCompatibleWritableRaster(int w, int h) {
        ...
        if (pixel_bits == 1 || pixel_bits == 2 || pixel_bits == 4) {
            raster = TRaster.createPackedRaster(TDataBuffer.TYPE_BYTE, w, h, 1,
                    pixel_bits, null);
        } else if (pixel_bits <= 8) {
            raster = TRaster.createInterleavedRaster(TDataBuffer.TYPE_BYTE, w, h,
                    1, null);
        } else if (pixel_bits <= 16) {
            raster = TRaster.createInterleavedRaster(TDataBuffer.TYPE_USHORT, w,
                    h, 1, null);
        } else {
    ...
    @Override
    public boolean isCompatibleSampleModel(TSampleModel sm) {
        if (sm == null) {
            return false;
        }
    ...
    public TSampleModel createCompatibleSampleModel(int w, int h) {
        if (pixel_bits == 1 || pixel_bits == 2 || pixel_bits == 4) {
            return new TMultiPixelPackedSampleModel(TDataBuffer.TYPE_BYTE, w, h,
                    pixel_bits);
        }
        int bandOffsets[] = new int[1];
        bandOffsets[0] = 0;
        return new TComponentSampleModel(transferType, w, h, 1, w,
                bandOffsets);

    }

    @Override
    public boolean isCompatibleRaster(TRaster raster) {
    ...
    private Object createDataObject(int colorMapIdx, Object pixel) {
        if (pixel == null) {
            ...
            pixel = ba;
                ...
                transferType == TDataBuffer.TYPE_USHORT) {
            short[] sa = (short[]) pixel;
            sa[0] = (short) colorMapIdx;
            pixel = sa;
        } else if (pixel instanceof int[]) {
    ...
    private static int validateTransferType(int transferType) {
        if (transferType != TDataBuffer.TYPE_BYTE &&
               transferType != TDataBuffer.TYPE_USHORT) {
            // awt.269=The transferType is not one of TDataBuffer.TYPE_BYTE or TDataBuffer.TYPE_USHORT
