...
public final class TGeneralPath implements TShape, Cloneable {

    public static final int WIND_EVEN_ODD = TPathIterator.WIND_EVEN_ODD;
    public static final int WIND_NON_ZERO = TPathIterator.WIND_NON_ZERO;
    ...
    class Iterator implements TPathIterator {
         ...
         */
        Iterator(TGeneralPath path, TAffineTransform at) {
            this.p = path;
            this.t = at;
        }
        ...
        public int currentSegment(double[] coords) {
            ...
            int count = TGeneralPath.pointShift[type];
    ...
    public TGeneralPath(TShape shape) {
        this(WIND_NON_ZERO, BUFFER_SIZE);
        TPathIterator p = shape.getPathIterator(null);
    ...
    void checkBuf(int pointCount, boolean checkMove) {
        if (checkMove && typeSize == 0) {
            ...
            throw new TIllegalPathStateException(Messages.getString("awt.20A")); //$NON-NLS-1$
        }
    ...
    public void moveTo(float x, float y) {
        if (typeSize > 0 && types[typeSize - 1] == TPathIterator.SEG_MOVETO) {
    ...
    public void closePath() {
        if (typeSize == 0 || types[typeSize - 1] != TPathIterator.SEG_CLOSE) {
            checkBuf(0, true);
            types[typeSize++] = TPathIterator.SEG_CLOSE;
        }
    }

    public void append(TShape shape, boolean connect) {
        TPathIterator p = shape.getPathIterator(null);
        append(p, connect);
    }

    public void append(TPathIterator path, boolean connect) {
        while (!path.isDone()) {
            ...
            case TPathIterator.SEG_MOVETO:
                ...
                if (types[typeSize - 1] != TPathIterator.SEG_CLOSE &&
    ...
    public TPoint2D getCurrentPoint() {
        ...
        return new TPoint2D.TFloat(points[j], points[j + 1]);
    }
    ...
    public TShape createTransformedShape(TAffineTransform t) {
        TGeneralPath p = (TGeneralPath)clone();
        if (t != null) {
            p.transform(t);
        }
        return p;
    ...
    public Object clone() {
        try {
            TGeneralPath p = (TGeneralPath) super.clone();
            p.types = types.clone();
