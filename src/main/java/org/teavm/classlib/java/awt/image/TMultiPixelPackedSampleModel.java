...
public class TMultiPixelPackedSampleModel extends TSampleModel {
    ...
    public TMultiPixelPackedSampleModel(int dataType, int w, int h,
        ...
        if(numberOfBits == 0) {
            ...
            throw new TRasterFormatException(Messages.getString("awt.20C")); //$NON-NLS-1$
        }
        this.pixelBitStride = numberOfBits;
        this.dataElementSize = TDataBuffer.getDataTypeSize(dataType);
        if(dataElementSize % pixelBitStride != 0) {
            ...
            throw new TRasterFormatException(Messages.getString("awt.20D")); //$NON-NLS-1$
        }

        if(dataBitOffset % numberOfBits != 0) {
            // awt.20E=Data Bit offset is not a multiple of pixel bit stride
            throw new TRasterFormatException(Messages.getString("awt.20E")); //$NON-NLS-1$
        }
    ...
    public TMultiPixelPackedSampleModel(int dataType, int w, int h,
        ...
        this(dataType, w, h, numberOfBits, (numberOfBits * w +
               ...
               TDataBuffer.getDataTypeSize(dataType), 0);
    }
    ...
    public boolean equals(Object o) {
        if ((o == null) || !(o instanceof TMultiPixelPackedSampleModel)) {
            return false;
        }

        TMultiPixelPackedSampleModel model = (TMultiPixelPackedSampleModel) o;
    ...
    public TSampleModel createSubsetSampleModel(int bands[]) {
        if (bands != null && bands.length != 1) {
            ...
            throw new TRasterFormatException(Messages.getString("awt.20F")); //$NON-NLS-1$
        }
    ...
    public TSampleModel createCompatibleSampleModel(int w, int h) {
        return new TMultiPixelPackedSampleModel(dataType, w, h, pixelBitStride);
    }
    ...
    public TDataBuffer createDataBuffer() {
        TDataBuffer dataBuffer = null;
        int size = scanlineStride * height;

        switch (dataType) {
        case TDataBuffer.TYPE_BYTE:
            dataBuffer = new TDataBufferByte(size + (dataBitOffset + 7) / 8);
            break;
        case TDataBuffer.TYPE_USHORT:
            dataBuffer = new TDataBufferUShort(size + (dataBitOffset + 15) / 16);
            break;
        case TDataBuffer.TYPE_INT:
            dataBuffer = new TDataBufferInt(size + (dataBitOffset + 31) / 32);
            break;
        }
        return dataBuffer;
    }
    ...
    public int getTransferType() {
        if (pixelBitStride > 16) {
            return TDataBuffer.TYPE_INT;
        } else if (pixelBitStride > 8) {
            return TDataBuffer.TYPE_USHORT;
        } else {
            return TDataBuffer.TYPE_BYTE;
        }
    }
    ...
    private void setSample(final int x, final int y, final Object obj,
            final TDataBuffer data, final int methodId, int s) {
        if ((x < 0) || (y < 0) || (x >= this.width) || (y >= this.height)) {
            // awt.63=Coordinates are not in bounds
