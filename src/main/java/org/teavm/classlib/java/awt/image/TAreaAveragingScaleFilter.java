...
public class TAreaAveragingScaleFilter extends TReplicateScaleFilter {

    private static final TColorModel rgbCM = TColorModel.getRGBdefault();
    private static final int averagingFlags = (TImageConsumer.TOPDOWNLEFTRIGHT |
    ...
    public TAreaAveragingScaleFilter(int width, int height) {
        super(width, height);
    }

    @Override
    public void setPixels(int x, int y, int w, int h, TColorModel model, int[] pixels, int off, int scansize) {
        if(reset) {
    ...
    @Override
    public void setPixels(int x, int y, int w, int h, TColorModel model, byte[] pixels, int off, int scansize) {
        if(reset) {
            super.setPixels(x, y, w, h, model, pixels, off, scansize);
        } else {
            setFilteredPixels(x, y, w, h, model, pixels, off, scansize);
     ...
     */
    private void setFilteredPixels(int x, int y, int w, int h, TColorModel model, Object pixels, int off, int scansize){
        if(!inited){
            initialize();
        }
