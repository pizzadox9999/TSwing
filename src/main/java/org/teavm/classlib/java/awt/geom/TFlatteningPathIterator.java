...
public class TFlatteningPathIterator implements TPathIterator {
    ...
    public TFlatteningPathIterator(TPathIterator path, double flatness) {
        this(path, flatness, BUFFER_LIMIT);
    }

    public TFlatteningPathIterator(TPathIterator path, double flatness, int limit) {
        if (flatness < 0.0) {
    ...
    void evaluate() {
        ...
        case SEG_QUADTO:
            ...
            while (bufSubdiv < bufLimit) {
                if (TQuadCurve2D.getFlatnessSq(buf, bufIndex) < flatness2) {
                    break;
                }
                ...
                TQuadCurve2D.subdivide(buf, bufIndex, buf, bufIndex - 4, buf, bufIndex);
        ...
        case SEG_CUBICTO:
            ...
            while (bufSubdiv < bufLimit) {
                if (TCubicCurve2D.getFlatnessSq(buf, bufIndex) < flatness2) {
                    break;
                }
                ...
                TCubicCurve2D.subdivide(buf, bufIndex, buf, bufIndex - 6, buf, bufIndex);
