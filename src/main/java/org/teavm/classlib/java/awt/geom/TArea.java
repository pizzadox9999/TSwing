...
public class TArea implements TShape, Cloneable {
	...
	public TArea(TShape s) {
		...
		for (TPathIterator pi = s.getPathIterator(null); 
		        !pi.isDone(); pi.next()) {
		...
		if ((rulesIndex != 0) && 
				(rules[rulesIndex - 1] != TPathIterator.SEG_CLOSE)) {
			rules[rulesIndex] = TPathIterator.SEG_CLOSE;
	...
	public void transform(TAffineTransform t) {
		copy(new TArea(t.createTransformedShape(this)), this);
	}

	public TArea createTransformedArea(TAffineTransform t) {
		return new TArea(t.createTransformedShape(this));
	}
	...
	public void intersect(TArea area) {
		if (area == null) {
		    return;
	...
	public void subtract(TArea area) {
 	...
 	public void exclusiveOr(TArea area) {
		TArea a = (TArea) clone();
		a.intersect(area);
		add(area);
		subtract(a);
	...
	private void addCurvePolygon(TArea area) {
        ...
        resultRules[resultRulesPos] = TPathIterator.SEG_MOVETO;
        ...
        resultRules[resultRulesPos++] = TPathIterator.SEG_CLOSE;
    ...
    private void addPolygon(TArea area) {
        ...
        do {
        	resultCoords[resultCoordPos++] = point.getX();
            resultCoords[resultCoordPos++] = point.getY();
            resultRules[resultRulesPos] = TPathIterator.SEG_LINETO;
        ...
        resultRules[resultRulesPos - 1] = TPathIterator.SEG_CLOSE;
        resultOffsets[resultRulesPos - 1] = resultCoordPos;
		coords = resultCoords;
		rules = resultRules;
 	...
 	private void intersectCurvePolygon(TArea area) {
		CurveCrossingHelper crossHelper = new CurveCrossingHelper(
				new double[][] {coords, area.coords }, 
				new int[] { coordsSize, area.coordsSize }, 
				new int[][] { rules, area.rules },
        ...
        do {
            ...
            if ((length == offset) && 
            	(nextPoint.getRule(isCurrentArea) != TPathIterator.SEG_LINETO) && 
                (nextPoint.getRule(isCurrentArea) != TPathIterator.SEG_CLOSE) &&
            	(point.getRule(isCurrentArea) != TPathIterator.SEG_LINETO) && 
            	(point.getRule(isCurrentArea) != TPathIterator.SEG_CLOSE)) {
        ...
        if (resultRules[resultRulesPos - 1] == TPathIterator.SEG_LINETO) {
        	resultRules[resultRulesPos - 1] = TPathIterator.SEG_CLOSE;
        } else {
	...
	private void intersectPolygon(TArea area) {
		CrossingHelper crossHelper = new CrossingHelper(new double[][] {coords, 
				                                        area.coords }, 
				                                        new int[] { coordsSize, 
				                                        area.coordsSize });
	...
	private void subtractCurvePolygon(TArea area) {
		CurveCrossingHelper crossHelper = new CurveCrossingHelper(
				new double[][] { coords, area.coords }, 
				new int[] { coordsSize, area.coordsSize }, 
				new int[][] { rules, area.rules },
        ...
        do {
            ...
            if (isCopyUntilZero) {
           	...
           	if ((length == offset) && 
           		((rules[offset] == TPathIterator.SEG_QUADTO) || 
           		 (rules[offset] == TPathIterator.SEG_CUBICTO))) {
	...
	private void subtractPolygon(TArea area) {
		CrossingHelper crossHelper = new CrossingHelper(new double[][] {coords, 
				                                        area.coords }, 
				                                        new int[] { coordsSize, 
				                                        area.coordsSize });
	...
	private int includeCoordsAndRules(int offset, int length, int[] rules,
		...
		if (length <= offset) {
			for (int i = resultRulesPos; i < resultRulesPos + 1; i++) {
				resultRules[i] = TPathIterator.SEG_LINETO;
			}
		} else {
			int j = resultRulesPos;
			for (int i = offset; i < length; i++) {
				resultRules[j++] = TPathIterator.SEG_LINETO;
			}
		}
		...
		for (int i = offset; i < length; i++) {
			...
			switch (rules[i]) {
			    ...
			    case TPathIterator.SEG_CLOSE:
				    ...
				    break;
			    case TPathIterator.SEG_QUADTO:
				    resultRules[resultRulesPos] = TPathIterator.SEG_QUADTO;
				    ...
				    break;
			    case TPathIterator.SEG_CUBICTO:
				    resultRules[resultRulesPos] = TPathIterator.SEG_CUBICTO;
	...
	private void copy(TArea src, TArea dst) {
    ...
    private int containsExact(double x, double y) {
        TPathIterator pi = getPathIterator(null);
    ...
    private double getAreaBoundsSquare() {
        TRectangle2D bounds = getBounds2D();
	...
	private class AreaPathIterator implements TPathIterator {

		TAffineTransform transform;
		TArea area;
		int curRuleIndex = 0;
		int curCoordIndex = 0;

		AreaPathIterator(TArea area) {
			this(area, null);
		}

		AreaPathIterator(TArea area, TAffineTransform t) {
			this.area = area;
			this.transform = t;
		}
