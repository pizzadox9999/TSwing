...
public abstract class TRGBImageFilter extends TImageFilter {

    protected TColorModel origmodel;

    protected TColorModel newmodel;

    protected boolean canFilterIndexColorModel;

    public TRGBImageFilter() {}

    public TIndexColorModel filterIndexColorModel(TIndexColorModel icm) {
        ...
        return new TIndexColorModel(bits, mapSize, filteredColorMap, 0,
                hasAlpha, trans, transferType);
    }

    public void substituteColorModel(TColorModel oldcm, TColorModel newcm) {
        origmodel = oldcm;
        newmodel = newcm;
    }
    ...
    public void setColorModel(TColorModel model) {
        if(model instanceof TIndexColorModel &&
            ...
            TColorModel filteredModel = filterIndexColorModel(icm);
            substituteColorModel(model, filteredModel);
            consumer.setColorModel(filteredModel);
        }else{
            consumer.setColorModel(TColorModel.getRGBdefault());
        }
    }
    ...
    public void filterRGBPixels(int x, int y, int w, int h, 
        ...
        consumer.setPixels(x, y, w, h, TColorModel.getRGBdefault(), 
                pixels, off, scansize);
    }
