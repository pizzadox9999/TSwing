...
public abstract class TGraphics {
    ...
    public TRectangle getClipBounds(TRectangle r) {
        ...
        if (clip != null) {
            // TODO: Can we get shape bounds without creating TRectangle object?
            TRectangle b = clip.getBounds();
            r.x = b.x;
            r.y = b.y;
            r.width = b.width;
            r.height = b.height;
    ...
    public boolean hitClip(int x, int y, int width, int height) {
        // TODO: Create package private method TRectangle.intersects(int, int, int, int);
        return getClipBounds().intersects(new TRectangle(x, y, width, height));
    }
    ...
    public abstract boolean drawImage(TImage img, int x, int y, TColor bgcolor, TImageObserver observer);

    public abstract boolean drawImage(TImage img, int x, int y, TImageObserver observer);

    public abstract boolean drawImage(TImage img, int x, int y, int width, int height, TColor bgcolor, TImageObserver observer);

    public abstract boolean drawImage(TImage img, int x, int y, int width, int height, TImageObserver observer);

    public abstract boolean drawImage(TImage img, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2, TColor bgcolor, TImageObserver observer);

    public abstract boolean drawImage(TImage img, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2, TImageObserver observer);
    ...
    public abstract void setXORMode(TColor color);
