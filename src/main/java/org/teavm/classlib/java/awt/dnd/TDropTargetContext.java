...
public class TDropTargetContext implements Serializable {
    ...
    final TDropTarget target;

    TDropTargetContextPeer peer;

    TDropTargetContext(TDropTarget target) {
        this.target = target;
    }
    ...
    public void addNotify(TDropTargetContextPeer peer) {
        this.peer = peer;
    }

    public TDropTarget getDropTarget() {
        return target;
    }

    protected Transferable getTransferable() throws TInvalidDnDOperationException {
        if (peer == null) {
            // awt.07=Transfer data is not available
            throw new TInvalidDnDOperationException(Messages.getString("awt.07")); //$NON-NLS-1$
        }
    ...
    public void dropComplete(boolean success) throws TInvalidDnDOperationException {
        if (peer != null) {
