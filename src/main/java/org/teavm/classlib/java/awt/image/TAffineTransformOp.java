...
public class TAffineTransformOp implements TBufferedImageOp, TRasterOp {
    ...
    public TAffineTransformOp(AffineTransform xform, RenderingHints hints) {
    ...
    public TAffineTransformOp(AffineTransform xform, int interp) {
        if (Math.abs(xform.getDeterminant()) <= Double.MIN_VALUE) {
            // awt.24F=Unable to invert transform {0}
            throw new TImagingOpException(Messages.getString("awt.24F", xform)); //$NON-NLS-1$
        }
    ...
    public TBufferedImage createCompatibleDestImage(TBufferedImage src, TColorModel destCM) {
        ...
        if (dstWidth <= 0 || dstHeight <= 0) {
            ...
            throw new TRasterFormatException(
                    Messages.getString("awt.251", dstWidth, dstHeight)); //$NON-NLS-1$
        }

        if (destCM != null) {
            return new TBufferedImage(destCM,
        ...
        TColorModel cm = src.getColorModel();

        // Interpolation other than NN doesn't make any sense for index color
        if (iType != TYPE_NEAREST_NEIGHBOR && cm instanceof TIndexColorModel) {
            return new TBufferedImage((int)dstWidth, (int)dstHeight, TBufferedImage.TYPE_INT_ARGB);
        }

        // OK, we can get source color model
        return new TBufferedImage(cm,
    ...
    public final TBufferedImage filter(TBufferedImage src, TBufferedImage dst) {
        ...
        if (
                srcCM instanceof TIndexColorModel &&
                (iType != TYPE_NEAREST_NEIGHBOR || srcCM.getPixelSize() % 8 != 0)
        ) {
            src = ((TIndexColorModel)srcCM).convertToIntDiscrete(src.getRaster(), true);
    ...
    private int ippFilter(TRaster src, TWritableRaster dst, int imageType) {
        ...
        switch (imageType) {
            ...
            case TBufferedImage.TYPE_BYTE_INDEXED: {
                channels = 1;
            ...
            default: {
                ...
                if (
                    ...
                    int dataTypeSize = TDataBuffer.getDataTypeSize(srcSM.getDataType()) / 8;

                    srcStride = ((TComponentSampleModel) srcSM).getScanlineStride() * dataTypeSize;
                    dstStride = ((TComponentSampleModel) dstSM).getScanlineStride() * dataTypeSize;
                } else if (
                    ...
                    if (sppsm1.getDataType() == TDataBuffer.TYPE_USHORT) {
                        return slowFilter(src, dst);
                    }
                    ...
                    int dataTypeSize = TDataBuffer.getDataTypeSize(sppsm1.getDataType()) / 8;
