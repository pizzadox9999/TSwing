...
public abstract class TSampleModel {
    ...
    public TSampleModel(int dataType, int w, int h, int numBands) {
        if (w <= 0 || h <= 0) {
        ...
        if (dataType < TDataBuffer.TYPE_BYTE ||
                dataType > TDataBuffer.TYPE_DOUBLE &&
                dataType != TDataBuffer.TYPE_UNDEFINED) {
    ...
    public abstract TSampleModel createSubsetSampleModel(int bands[]);

    public abstract TSampleModel createCompatibleSampleModel(int a0, int a1);
    ...
    public float[] getPixel(int x, int y, float fArray[], TDataBuffer data) {
        if (x < 0 || y < 0 || x >= this.width || y >= this.height) {
            // awt.63=Coordinates are not in bounds
            throw new ArrayIndexOutOfBoundsException(Messages.getString("awt.63")); //$NON-NLS-1$
        }
    ...
    public void setPixel(int x, int y, float fArray[], TDataBuffer data) {
        if (x < 0 || y < 0 || x >= this.width || y >= this.height) {
            // awt.63=Coordinates are not in bounds
            throw new ArrayIndexOutOfBoundsException(Messages.getString("awt.63")); //$NON-NLS-1$
        }
    ...
    public double[] getPixel(int x, int y, double dArray[], TDataBuffer data) {
        if (x < 0 || y < 0 || x >= this.width || y >= this.height) {
            // awt.63=Coordinates are not in bounds
            throw new ArrayIndexOutOfBoundsException(Messages.getString("awt.63")); //$NON-NLS-1$
        }
    ...
    public void setPixel(int x, int y, double dArray[], TDataBuffer data) {
        if (x < 0 || y < 0 || x >= this.width || y >= this.height) {
            // awt.63=Coordinates are not in bounds
            throw new ArrayIndexOutOfBoundsException(Messages.getString("awt.63")); //$NON-NLS-1$
        }
    ...
    public double[] getSamples(int x, int y, int w, int h, int b,
            double dArray[], TDataBuffer data) {
        double samples[];
        int idx = 0;
