...
public abstract class TArc2D extends TRectangularShape {

    public final static int OPEN = 0;
    public final static int CHORD = 1;
    public final static int PIE = 2;

    public static class TFloat extends TArc2D {
        ...
        public TFloat(int type) {
            super(type);
        }

        public TFloat(float x, float y, float width, float height, float start, float extent, int type) {
            super(type);
            this.x = x;
            this.y = y;
            this.width = width;
        ...
        public TFloat(TRectangle2D bounds, float start, float extent, int type) {
            super(type);
    ...
    public double getY() {
        ...
        protected TRectangle2D makeBounds(double x, double y, double width, double height) {
            return new TRectangle2D.TFloat((float)x, (float)y, (float)width, (float)height);
        }

    }

    public static class TDouble extends TArc2D {
        ...
        public TDouble(TRectangle2D bounds, double start, double extent, int type) {
            super(type);
            this.x = bounds.getX();
            this.y = bounds.getY();
    ...
    class Iterator implements TPathIterator {
         ...
         */
        Iterator(TArc2D a, TAffineTransform t) {
            if (width < 0 || height < 0) {
                arcCount = 0;
                lineCount = 0;
                index = 1;
            ...
            lineCount = 0;
            if (type == TArc2D.CHORD) {
                lineCount++;
            } else if (type == TArc2D.PIE) {
                lineCount += 2;
            }
        }
    ...
    protected TArc2D(int type) {
        setArcType(type);
    }

    protected abstract TRectangle2D makeBounds(double x, double y, double width, double height);
    ...
    public void setArc(TPoint2D point, TDimension2D size, double start, double extent, int type) {
        setArc(point.getX(), point.getY(), size.getWidth(), size.getHeight(), start, extent, type);
    }

    public void setArc(TRectangle2D rect, double start, double extent, int type) {
        setArc(rect.getX(), rect.getY(), rect.getWidth(), rect.getHeight(), start, extent, type);
    }

    public void setArc(TArc2D arc) {
    ...
    public void setArcByTangent(TPoint2D p1, TPoint2D p2, TPoint2D p3, double radius) {
    ...
    public void setAngleStart(TPoint2D point) {
    ...
    public void setAngles(TPoint2D p1, TPoint2D p2) {
    ...
    public boolean contains(double px, double py) {
        ...
        TLine2D l = new TLine2D.Double(getStartPoint(), getEndPoint());
    ...
    @Override
    public boolean contains(TRectangle2D rect) {
