...
public class TDirectColorModel extends TPackedColorModel {
    ...
    public Object getDataElements(int rgb, Object pixel) {
        if (equals(TColorModel.getRGBdefault())) {
            int ia[];
            if (pixel == null) {
                ia = new int[1];
            } else {
    ...
    @Override
    public final TColorModel coerceData(TWritableRaster raster,
        ...
        return new TDirectColorModel(cs, pixel_bits, componentMasks[0],
    ...
    @Override
    public final TWritableRaster createCompatibleWritableRaster(int w, int h) {
        if (w <= 0 || h <= 0) {
        ...
        if (pixel_bits > 16) {
            return TRaster.createPackedRaster(TDataBuffer.TYPE_INT, w, h,
                    bandMasks, null);
        } else if (pixel_bits > 8) {
            return TRaster.createPackedRaster(TDataBuffer.TYPE_USHORT, w, h,
                    bandMasks, null);
        } else {
            return TRaster.createPackedRaster(TDataBuffer.TYPE_BYTE, w, h,
                    bandMasks, null);
        }
    }
