...
public class TDataFlavor implements Externalizable, Cloneable {
     ...
     */
    @Deprecated
    public static final TDataFlavor plainTextFlavor =
            new TDataFlavor("text/plain; charset=unicode; class=java.io.InputStream", //$NON-NLS-1$
                    "Plain Text"); //$NON-NLS-1$

    public static final TDataFlavor stringFlavor =
            new TDataFlavor("application/x-java-serialized-object; class=java.lang.String", //$NON-NLS-1$
                    "Unicode String"); //$NON-NLS-1$

    public static final TDataFlavor imageFlavor =
            new TDataFlavor("image/x-java-image; class=java.awt.Image", //$NON-NLS-1$
                    "Image"); //$NON-NLS-1$

    public static final TDataFlavor javaFileListFlavor =
            new TDataFlavor("application/x-java-file-list; class=java.util.List", //$NON-NLS-1$
    ...
    private static TDataFlavor plainUnicodeFlavor = null;

    private String humanPresentableName;
    private Class<?> representationClass;
    private TMimeTypeProcessor.MimeType mimeInfo;

    public static final TDataFlavor getTextPlainUnicodeFlavor() {
        if (plainUnicodeFlavor == null) {
            plainUnicodeFlavor = new TDataFlavor("text/plain" //$NON-NLS-1$
    ...
    public TDataFlavor(Class<?> representationClass, String humanPresentableName) {
        mimeInfo = new TMimeTypeProcessor.MimeType("application", "x-java-serialized-object"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public TDataFlavor(String mimeType, String humanPresentableName) {
        try {
    ...
    public TDataFlavor(String mimeType) throws ClassNotFoundException {
        init(mimeType, null, null);
    }

    public TDataFlavor(String mimeType, String humanPresentableName, ClassLoader classLoader)
            throws ClassNotFoundException
    {
            ...
            throws ClassNotFoundException
    {
        ...
        try {
            mimeInfo = TMimeTypeProcessor.parse(mimeType);
    ...
    TMimeTypeProcessor.MimeType getMimeInfo() {
        return mimeInfo;
    }
    ...
    public String getMimeType() {
        return (mimeInfo != null) ? TMimeTypeProcessor.assemble(mimeInfo) : null;
    }
    ...
    public final boolean isMimeTypeEqual(TDataFlavor dataFlavor) {
    ...
    public boolean isMimeTypeEqual(String mimeType) {
        try {
            return mimeInfo.equals(TMimeTypeProcessor.parse(mimeType));
        } catch (IllegalArgumentException e) {
            return false;
        }
    }
            ...
            throws IOException, ClassNotFoundException
    {
        ...
        mimeInfo = (TMimeTypeProcessor.MimeType) is.readObject();
    ...
    public boolean equals(Object o) {
        if ((o == null) || !(o instanceof TDataFlavor)) {
            return false;
        }
        return equals((TDataFlavor) o);
    }

    public boolean equals(TDataFlavor that) {
        if (that == this) {
            return true;
        }
        if (that == null) {
    ...
    public boolean match(TDataFlavor that) {
        return equals(that);
    }
    ...
    public Reader getReaderForText(TTransferable transferable)
    ...
    public static final TDataFlavor selectBestTextFlavor(TDataFlavor[] availableFlavors) {
        if (availableFlavors == null) {
            return null;
        }

        List<List<TDataFlavor>> sorted = sortTextFlavorsByType(new LinkedList<TDataFlavor>(Arrays.asList(availableFlavors)));

        if (sorted.isEmpty()) {
            return null;
        }

        List<TDataFlavor> bestSorted = sorted.get(0);
    ...
    private static TDataFlavor selectBestFlavorWCharset(List<TDataFlavor> list) {
        List<TDataFlavor> best;
    ...
    private static TDataFlavor selectBestByCharset(List<TDataFlavor> list) {
        List<TDataFlavor> best;
    ...
    private static List<TDataFlavor> selectBestByAlphabet(List<TDataFlavor> list) {
        String charsets[] = new String[list.size()];
        LinkedList<TDataFlavor> best = new LinkedList<TDataFlavor>();
    ...
    private static List<TDataFlavor> getFlavors(List<TDataFlavor> list, String[] charset) {
        LinkedList<TDataFlavor> sublist = new LinkedList<TDataFlavor>();

        for (Iterator<TDataFlavor> i = list.iterator(); i.hasNext();) {
            TDataFlavor flavor = i.next();
    ...
    private static TDataFlavor selectBestFlavorWOCharset(List<TDataFlavor> list) {
        List<TDataFlavor> best;
    ...
    private static List<TDataFlavor> getFlavors(List<TDataFlavor> list, Class<?> klass) {
        LinkedList<TDataFlavor> sublist = new LinkedList<TDataFlavor>();
    ...
    private static List<List<TDataFlavor>> sortTextFlavorsByType(List<TDataFlavor> availableFlavors) {
        LinkedList<List<TDataFlavor>> list = new LinkedList<List<TDataFlavor>>();

        for (String element : sortedTextFlavors) {
            List<TDataFlavor> subList = fetchTextFlavors(availableFlavors, element);
    ...
    private static List<TDataFlavor> fetchTextFlavors(List<TDataFlavor> availableFlavors, String mimeType) {
        LinkedList<TDataFlavor> list = new LinkedList<TDataFlavor>();

        for (Iterator<TDataFlavor> i = availableFlavors.iterator(); i.hasNext();) {
            TDataFlavor flavor = i.next();
