...
class PrinterJobImpl extends TPrinterJob {

    private final PrintRequestAttributeSet attributes;

    private Doc                            doc;
    private TPageFormat                     format;
    ...
    @Override
    public TPageFormat defaultPage(final TPageFormat page) {
        if (page != null) {
            format = (TPageFormat) page.clone();
    ...
    @Override
    public TPageFormat pageDialog(final PrintRequestAttributeSet attributes)
    ...
    @Override
    public TPageFormat pageDialog(final TPageFormat page)
                    throws HeadlessException {
        final TPageFormat format = pageDialog(formatToAttrs(page));
        return format != null ? format : page;
    }

    @Override
    public void print() throws TPrinterException {
        print(attributes);
    }
    ...
    public void print(final PrintRequestAttributeSet attributes)
        ...
        if (doc == null) {
            ...
            throw new TPrinterException(Messages.getString("awt.29A")); //$NON-NLS-1$
        }
        ...
        try {
            ...
            final TPrinterException pe = new TPrinterException();
            pe.initCause(ex);
            throw pe;
        }
    }
    ...
    @Override
    public void setPageable(final TPageable document)
    ...
    @Override
    public void setPrintable(final TPrintable painter) {
        setPrintable(painter, null);
    }

    @Override
    public void setPrintable(final TPrintable painter, final TPageFormat format) {
        if (format != null) {
    ...
    @Override
    public TPageFormat validatePage(final TPageFormat page) {
        return page;
    }
    ...
    private static PrintRequestAttributeSet formatToAttrs(
        ...
        if (format != null) {
            ...
            case TPageFormat.PORTRAIT:
                attributes.add(OrientationRequested.PORTRAIT);
                break;
            case TPageFormat.LANDSCAPE:
                attributes.add(OrientationRequested.LANDSCAPE);
                break;
            case TPageFormat.REVERSE_LANDSCAPE:
    ...
    private static TPageFormat attrsToFormat(
                    final PrintRequestAttributeSet attributes) {
        if (attributes == null) {
            return new TPageFormat();
        }

        final TPageFormat format = new TPageFormat();
        final TPaper paper = new TPaper();
        ...
        if (orient != null) {
            if (orient.equals(OrientationRequested.LANDSCAPE)) {
                format.setOrientation(TPageFormat.LANDSCAPE);
            } else if (orient.equals(OrientationRequested.REVERSE_LANDSCAPE)) {
                format.setOrientation(TPageFormat.REVERSE_LANDSCAPE);
            }
        }
    ...
    private static class PrintableWrapper implements TPageable {
        private final TPrintable  p;
        private final TPageFormat format;

        PrintableWrapper(final TPrintable p, final TPageFormat format) {
            this.p = p;
            this.format = (format != null) ? format : new TPageFormat();
        }

        public int getNumberOfPages() {
            return TPageable.UNKNOWN_NUMBER_OF_PAGES;
        }
