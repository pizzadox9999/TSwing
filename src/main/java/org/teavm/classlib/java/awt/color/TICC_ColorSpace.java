...
public class TICC_ColorSpace extends TColorSpace {
    private static final long serialVersionUID = 3455889114070431483L;

    // Need to keep compatibility with serialized form
    private static final ObjectStreamField[]
      serialPersistentFields = {
        new ObjectStreamField("thisProfile", TICC_Profile.class), //$NON-NLS-1$
    ...
    */
    ...
    private TICC_Profile profile = null;
    ...
    private TICC_ColorSpace resolvedDeserializedInst;

    public TICC_ColorSpace(TICC_Profile pf) {
        super(pf.getColorSpaceType(), pf.getNumComponents());

        int pfClass = pf.getProfileClass();

        switch (pfClass) {
            case TICC_Profile.CLASS_COLORSPACECONVERSION:
            case TICC_Profile.CLASS_DISPLAY:
            case TICC_Profile.CLASS_OUTPUT:
            case TICC_Profile.CLASS_INPUT:
    ...
    public TICC_Profile getProfile() {
        if (profile instanceof TICC_ProfileStub) {
            profile = ((TICC_ProfileStub) profile).loadProfile();
        }
    ...
    public float[] toRGB(float[] colorvalue) {
        if (toRGBTransform == null) {
            TICC_Profile sRGBProfile =
                ((TICC_ColorSpace) TColorSpace.getInstance(CS_sRGB)).getProfile();
            TICC_Profile[] profiles = {getProfile(), sRGBProfile};
    ...
    public float[] toCIEXYZ(float[] colorvalue) {
        if (toXYZTransform == null) {
            TICC_Profile xyzProfile =
                ((TICC_ColorSpace) TColorSpace.getInstance(CS_CIEXYZ)).getProfile();
            TICC_Profile[] profiles = {getProfile(), xyzProfile};
            try {
                int[] intents = {
                        TICC_Profile.icRelativeColorimetric,
                        TICC_Profile.icPerceptual};
                toXYZTransform = new ICC_Transform(profiles, intents);
            } catch (TCMMException e) { // No such tag, use what we can
    ...
    public float[] fromRGB(float[] rgbvalue) {
        if (fromRGBTransform == null) {
            TICC_Profile sRGBProfile =
                ((TICC_ColorSpace) TColorSpace.getInstance(CS_sRGB)).getProfile();
            TICC_Profile[] profiles = {sRGBProfile, getProfile()};
    ...
    public float[] fromCIEXYZ(float[] xyzvalue) {
        if (fromXYZTransform == null) {
            TICC_Profile xyzProfile =
                ((TICC_ColorSpace) TColorSpace.getInstance(CS_CIEXYZ)).getProfile();
            TICC_Profile[] profiles = {xyzProfile, getProfile()};
            try {
                int[] intents = {
                        TICC_Profile.icPerceptual,
                        TICC_Profile.icRelativeColorimetric};
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        resolvedDeserializedInst =
                new TICC_ColorSpace((TICC_Profile) fields.get("thisProfile", null)); //$NON-NLS-1$
    }
