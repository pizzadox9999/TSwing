...
public class TPixelGrabber implements TImageConsumer {
    ...
    private static final int ALL_BITS = (TImageObserver.FRAMEBITS |
            TImageObserver.ALLBITS);

    private static final int GRABBING_STOP = ALL_BITS | TImageObserver.ERROR;



    public TPixelGrabber(TImageProducer ip, int x, int y, int w, int h, int[] pix,
            int off, int scansize) {
        initialize(ip, x, y, w, h, pix, off, scansize, true);
    }

    public TPixelGrabber(TImage img, int x, int y, int w, int h, int[] pix,
            int off, int scansize) {
        initialize(img.getSource(), x, y, w, h, pix, off, scansize, true);
    }

    public TPixelGrabber(TImage img, int x, int y, int w, int h, boolean forceRGB) {
    ...
    public void setPixels(int srcX, int srcY, int srcW, int srcH,
            TColorModel model, byte[] pixels, int srcOff, int srcScan) {
        if(srcY < Y){
            int delta = Y - srcY;
            if(delta >= height) {
                return;
        ...
        case DATA_TYPE_UNDEFINED:
            cm = model;
            if(model != TColorModel.getRGBdefault()){
    ...
    public void setPixels(int srcX, int srcY, int srcW, int srcH,
            TColorModel model, int[] pixels, int srcOff, int srcScan) {
        ...
        case DATA_TYPE_UNDEFINED:
            ...
            iData = new int[width * height];
            dataType = DATA_TYPE_INT;
            isRGB = (cm == TColorModel.getRGBdefault());
    ...
    public synchronized TColorModel getColorModel() {
        return cm;
    }
    ...
    throws InterruptedException {
        ...
        if(!isGrabbing){
            isGrabbing = true;
            grabberStatus &= ~TImageObserver.ABORT;
    ...
    public void setDimensions(int w, int h) {
        ...
        grabberStatus |= TImageObserver.WIDTH | TImageObserver.HEIGHT;
    ...
    public synchronized void imageComplete(int status) {
        ...
        case IMAGEABORTED:
            ...
            break;
        case IMAGEERROR:
            grabberStatus |= TImageObserver.ERROR | TImageObserver.ABORT;
            break;
        case SINGLEFRAMEDONE:
            grabberStatus |= TImageObserver.FRAMEBITS;
            break;
        case STATICIMAGEDONE:
            grabberStatus |= TImageObserver.ALLBITS;
            break;
        default:
    ...
    private void initialize(TImageProducer ip, int x, int y, int w, int h,
