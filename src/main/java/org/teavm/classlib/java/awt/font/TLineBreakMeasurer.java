...
public final class TLineBreakMeasurer {
    private TTextMeasurer tm = null;
    private BreakIterator bi = null;
    private int position = 0;
    int maxpos = 0;

    public TLineBreakMeasurer(AttributedCharacterIterator text, TFontRenderContext frc) {
        this(text, BreakIterator.getLineInstance(), frc);
    }

    public TLineBreakMeasurer(
            AttributedCharacterIterator text,
            BreakIterator bi,
            TFontRenderContext frc
    ) {
        tm = new TTextMeasurer(text, frc);
        this.bi = bi;
    ...
    public TTextLayout nextLayout(float wrappingWidth, int offsetLimit, boolean requireNextWord) {
        if (position == maxpos) {
            return null;
        }
        ...
        TTextLayout layout = tm.getLayout(position, nextPosition);
        position = nextPosition;
        return layout;
    }

    public TTextLayout nextLayout(float wrappingWidth) {
