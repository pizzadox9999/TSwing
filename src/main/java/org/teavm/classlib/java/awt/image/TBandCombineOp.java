...
public class TBandCombineOp implements TRasterOp {
    ...
    public TBandCombineOp(float matrix[][], RenderingHints hints) {
    ...
    public TWritableRaster filter(TRaster src, TWritableRaster dst) {
        int numBands = src.getNumBands();
    ...
    private final SampleModelInfo checkSampleModel(TSampleModel sm) {
        SampleModelInfo ret = new SampleModelInfo();

        if (sm instanceof TPixelInterleavedSampleModel) {
            // Check PixelInterleavedSampleModel
            if (sm.getDataType() != TDataBuffer.TYPE_BYTE) {
                return null;
            }

            ret.channels = sm.getNumBands();
            ret.stride = ((TComponentSampleModel) sm).getScanlineStride();
            ret.channelsOrder = ((TComponentSampleModel) sm).getBandOffsets();

        } else if (sm instanceof TSinglePixelPackedSampleModel) {
            // Check SinglePixelPackedSampleModel
            TSinglePixelPackedSampleModel sppsm1 = (TSinglePixelPackedSampleModel) sm;

            ret.channels = sppsm1.getNumBands();
            if (sppsm1.getDataType() != TDataBuffer.TYPE_INT) {
                return null;
            }
    ...
    private final int slowFilter(TRaster src, TWritableRaster dst) {
        int res = 0;
    ...
    private int verySlowFilter(TRaster src, TWritableRaster dst) {
        int numBands = src.getNumBands();
    ...
    private int ippFilter(TRaster src, TWritableRaster dst) {
        ...
        if (
            ...
            // Check PixelInterleavedSampleModel
            TPixelInterleavedSampleModel pism1 = (TPixelInterleavedSampleModel) srcSM;
            TPixelInterleavedSampleModel pism2 = (TPixelInterleavedSampleModel) dstSM;
            ...
            int dataTypeSize = TDataBuffer.getDataTypeSize(srcDataType) / 8;
