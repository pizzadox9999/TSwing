...
public final class TTextMeasurer implements Cloneable {
    AttributedCharacterIterator aci;
    TFontRenderContext frc;
    TextRunBreaker breaker = null;
    TextMetricsCalculator tmc = null;

    public TTextMeasurer(AttributedCharacterIterator text, TFontRenderContext frc) {
        this.aci = text;
        this.frc = frc;
    ...
    protected Object clone() {
        return new TTextMeasurer((AttributedCharacterIterator) aci.clone(), frc);
    }

    public TTextLayout getLayout(int start, int limit) {
        breaker.pushSegments(start - aci.getBeginIndex(), limit - aci.getBeginIndex());

        breaker.createAllSegments();
        TTextLayout layout = new TTextLayout((TextRunBreaker) breaker.clone());
