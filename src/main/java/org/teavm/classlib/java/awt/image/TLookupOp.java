...
public class TLookupOp implements TBufferedImageOp, TRasterOp {
    private final TLookupTable lut;
    ...
    public TLookupOp(TLookupTable lookup, RenderingHints hints) {
        if (lookup == null){
            throw new NullPointerException(Messages.getString("awt.01", "lookup")); //$NON-NLS-1$ //$NON-NLS-2$
        }
        lut = lookup;
        this.hints = hints;
        canUseIpp = lut instanceof TByteLookupTable || lut instanceof TShortLookupTable;
    }

    public final TLookupTable getTable() {
        return lut;
    }
    ...
    public TBufferedImage createCompatibleDestImage(TBufferedImage src, TColorModel dstCM) {
        if (dstCM == null) {
            ...
            if (dstCM instanceof TComponentColorModel) {
                int transferType = dstCM.getTransferType();
                if (lut instanceof TByteLookupTable) {
                    transferType = TDataBuffer.TYPE_BYTE;
                } else if (lut instanceof TShortLookupTable) {
                    transferType = TDataBuffer.TYPE_SHORT;
                }

                dstCM = new TComponentColorModel(
                        dstCM.cs,
                        dstCM.hasAlpha(),
    ...
    public final TWritableRaster filter(TRaster src, TWritableRaster dst) {
        if (dst == null) {
        ...
        // TODO
        // if (!canUseIpp || ippFilter(src, dst, TBufferedImage.TYPE_CUSTOM, false) != 0)
    ...
    public final TBufferedImage filter(TBufferedImage src, TBufferedImage dst) {
        ...
        if (dst == null) {
            ...
            if (!srcCM.equals(dst.getColorModel())) {
                // Treat TBufferedImage.TYPE_INT_RGB and
                // TBufferedImage.TYPE_INT_ARGB as same
                if (!((src.getType() == TBufferedImage.TYPE_INT_RGB || src
                        .getType() == TBufferedImage.TYPE_INT_ARGB) && (dst
                        .getType() == TBufferedImage.TYPE_INT_RGB || dst
                        .getType() == TBufferedImage.TYPE_INT_ARGB))) {
                    finalDst = dst;
                    dst = createCompatibleDestImage(src, null);
                }
            }
    ...
    private final int slowFilter(TRaster src, TWritableRaster dst, boolean skipAlpha) {
        ...
        if (lut instanceof TByteLookupTable){
            byte[][] byteData = ((TByteLookupTable)lut).getTable();
            ...
            short[][] shortData  = ((TShortLookupTable)lut).getTable();
            pixels = src.getPixels(minSrcX, minSrcY, srcWidth, srcHeight, pixels);
    ...
    private final void createByteLevels(
        ...
        byte data[][] = ((TByteLookupTable)lut).getTable();
    ...
    private final void createShortLevels(
        ...
        short data[][] = ((TShortLookupTable)lut).getTable();
        int nLevels = data[0].length;
        int offset = lut.getOffset();
    ...
    private final int ippFilter(
            TRaster src, TWritableRaster dst,
        ...
        switch (imageType) {
            ...
            case TBufferedImage.TYPE_USHORT_GRAY:
            ...
            default: {
                ...
                if (
                    ...
                    channelsOrder = ((TComponentSampleModel) srcSM).getBandOffsets();
                } else if (
                        srcSM instanceof TSinglePixelPackedSampleModel &&
                        dstSM instanceof TSinglePixelPackedSampleModel
                ) {
                    // Check SinglePixelPackedSampleModel
                    TSinglePixelPackedSampleModel sppsm1 =
                            (TSinglePixelPackedSampleModel) srcSM;
                    TSinglePixelPackedSampleModel sppsm2 =
                            (TSinglePixelPackedSampleModel) dstSM;
