...
public class TColorConvertOp implements TBufferedImageOp, TRasterOp {
    ...
    public TColorConvertOp(ColorSpace srcCS, ColorSpace dstCS, RenderingHints hints) {
    ...
    public TColorConvertOp(ICC_Profile profiles[], RenderingHints hints) {
        if (profiles == null) {
    ...
    public TColorConvertOp(ColorSpace cs, RenderingHints hints) {
        if (cs == null) {
            throw new NullPointerException(Messages.getString("awt.25B")); //$NON-NLS-1$
        }
    ...
    public TColorConvertOp(RenderingHints hints) {
        renderingHints = hints;
    }
    ...
    public TBufferedImage createCompatibleDestImage(TBufferedImage src, TColorModel destCM) {
        ...
        TColorModel dstCM = new TComponentColorModel(dstCS, 
                srcCM.hasAlpha(), 
                srcCM.isAlphaPremultiplied(),
                srcCM.getTransparency(),
                srcCM.getTransferType());
        
        return new TBufferedImage(dstCM,
    ...
    public final TBufferedImage filter(TBufferedImage src, TBufferedImage dst) {
        ...
        if (srcCM instanceof TIndexColorModel) {            
            src = ((TIndexColorModel) srcCM).convertToIntDiscrete(src.getRaster(), false);
        }
        ...
        if (dst != null) {
          ...
          if (src.getWidth() != dst.getWidth() ||
            ...
            if (dst.getColorModel() instanceof TIndexColorModel) {
                isDstIndex = true;
        ...
        TColorModel dstCM = res.getColorModel();
    ...
    public TWritableRaster createCompatibleDestRaster(TRaster src) {
        ...
        if (dstDataType != TDataBuffer.TYPE_BYTE &&
           dstDataType != TDataBuffer.TYPE_SHORT) {
                dstDataType = TDataBuffer.TYPE_SHORT;
        }
        
        return TRaster.createInterleavedRaster(
                dstDataType, 
                src.getWidth(),
                src.getHeight(),
                nComps,
