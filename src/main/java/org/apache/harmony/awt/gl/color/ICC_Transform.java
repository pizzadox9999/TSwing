...
public class ICC_Transform {
    ...
    private TICC_Profile src;
    private TICC_Profile dst;
     ...
     */
    public ICC_Transform(TICC_Profile[] profiles, int[] renderIntents) {
     ...
     * @param profiles - list of ICC profiles
     */
    public ICC_Transform(TICC_Profile[] profiles) {
        int numProfiles = profiles.length;
        int[] renderingIntents = new int[numProfiles];

        // Default is perceptual
        int currRenderingIntent = TICC_Profile.icPerceptual;

        // render as colorimetric for output device
        if (profiles[0].getProfileClass() == TICC_Profile.CLASS_OUTPUT) {
            currRenderingIntent = TICC_Profile.icRelativeColorimetric;
        }
        ...
        for (int i = 0; i < numProfiles; i++) {
            ...
            if (i != 0 &&
               i != numProfiles - 1 &&
               profiles[i].getProfileClass() == TICC_Profile.CLASS_ABSTRACT
            ) {
                currRenderingIntent = TICC_Profile.icPerceptual;
            }
