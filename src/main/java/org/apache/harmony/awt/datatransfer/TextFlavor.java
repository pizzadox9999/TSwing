...
public class TextFlavor {
    ...
    public static void addUnicodeClasses(TSystemFlavorMap fm,
                                         String nat,
                                         String subType) {
        for (int i = 0; i < unicodeTextClasses.length; i++) {
            ...
            TDataFlavor f = new TDataFlavor(type + params, type);
            fm.addFlavorForUnencodedNative(nat, f);
            fm.addUnencodedNativeForFlavor(f, nat);
        }
    }

    public static void addCharsetClasses(TSystemFlavorMap fm, 
                                         String nat,
                                         String subType, 
                                         String charset) {
