...
public class IMManager {
    ...
    private static class IMSelection extends PopupMenu implements ItemListener {        
        ...
        private class IMenuItem extends CheckboxMenuItem {
            ...
            private final TInputMethodDescriptor getDesc() {
        ...
        private class IMSubmenu extends Menu {
            private final TInputMethodDescriptor desc;
            IMSubmenu(TInputMethodDescriptor imd) throws HeadlessException {
        ...
        public IMSelection() {            
            Iterator<TInputMethodDescriptor> it = getIMDescriptors().iterator();
        ...
        private void addIM(Iterator<TInputMethodDescriptor> it) {
            TInputMethodDescriptor desc = it.next();
            IMSubmenu subMenu = new IMSubmenu(desc);
            add(subMenu);
        }
    
        private void addNativeIM(Iterator<TInputMethodDescriptor> it) {
            if (it.hasNext()) {
                addIM(it);
            }
        ...
        private void show(Component origin, InputMethodContext imc) {
            ...
            for (int i = 0; i < getItemCount(); i++) {
                ...
                TInputMethodDescriptor desc = subMenu.getDesc();
                if (desc == null) {
                    continue;
                }
    ...
    private static List<TInputMethodDescriptor> imd; // available IM descriptors
    ...
    static List<TInputMethodDescriptor> getIMDescriptors() {
        if (imd == null) {
     ...
     * Service.providers(TInputMethodDescriptor.class)
     * @return list of input method descriptors
     */
    private static List<TInputMethodDescriptor> loadIMDescriptors() {
        String nm = SERVICES + TInputMethodDescriptor.class.getName();
        Enumeration<URL> en;
        LinkedList<TInputMethodDescriptor> imdList = new LinkedList<TInputMethodDescriptor>();
        ...
        try {
            ...
            while (en.hasMoreElements()) {
                ...
                while (str != null) {
                    ...
                    if (str.length() > 0) {
                        imdList.add((TInputMethodDescriptor)cl.loadClass(str).newInstance());
                    }
                    str = br.readLine();
                }
    ...
    private static void showIMPopup(InputMethodContext imc, Window parent) {
        List<TInputMethodDescriptor> descriptors = getIMDescriptors();
